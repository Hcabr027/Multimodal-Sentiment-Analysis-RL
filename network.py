...
class DPSR(nn.Module): # Deep Phase Space Reconstruction: focus on important parts of the input sequence within each modality 
...
class Model(nn.Module): # text and vision linear projections, call DPSR block get modality specific representation, 
                        # calculate correlation loss to encorage disentanglement of features within each modality.
    def __init__(self, config):
        ...
        self.dim_t, self.dim_v = config.dim_t, config.dim_v
    ...
    def forward(self, text, visual):
        ...
        ###disentangle
        x_t, x_v = self.encoder_t(text), self.encoder_v(visual) ##[l,b, emb_size]
        ...
        x_v = self.dpsr_v(x_v)
        ###dpsr loss
        x_t_norm, x_v_norm = F.normalize(x_t, dim=2), F.normalize(x_v, dim=2)
        corr_t, corr_v = torch.matmul(x_t_norm, x_t_norm.permute(0, 2, 1)), torch.matmul(x_v_norm, x_v_norm.permute(0, 2, 1))
        
        corr_t, corr_v = (corr_t+1)/2, (corr_v+1)/2  #180

        w = self.weight(self.length).unsqueeze(0).cuda()
        w = w.repeat(self.batch, 1, 1)
        corr_t, corr_v = torch.mul(corr_t, w), torch.mul(corr_v, w)
        corr_t, corr_v = torch.triu(corr_t, diagonal=1), torch.triu(corr_v, diagonal=1)
        corrt, corrv = torch.sum(corr_t, (1, 2)), torch.sum(corr_v, (1, 2))
        corrt_mean, corrv_mean = corrt / (self.length * (self.length - 1) / 2), corrv / (self.length * (self.length - 1) / 2)
        corr_loss = (corrt_mean  + corrv_mean) / 2

        return torch.cat((x_t, x_v), 1), corr_loss


class Actor(nn.Module): # which modality should focus advise function and output label 
    def __init__(self, config):
    ...
    def generate_advise(self, text, vision): 
        sim_tv, sim_vt = self.similarity_matrix(text,  vision)
        diff_tv,  diff_vt = -sim_tv, -sim_vt
        ###text-center
        sim_tt = torch.ones_like(sim_tv)
        tt, tv, tv_neg = torch.sum(sim_tt, dim= [1, 2]), torch.sum(sim_tv, dim= [1, 2]), torch.sum(diff_tv, dim= [1, 2])
        scale_t = torch.nn.Softmax(1)(torch.cat((tt.unsqueeze(1), tv.unsqueeze(1), tv_neg.unsqueeze(1)), 1))
        ...
        ###vision-center
        sim_vv = torch.ones_like(sim_tv)
        vv, vt, vt_neg,  = torch.sum(sim_vv, dim= [1, 2]), torch.sum(sim_vt, dim= [1, 2]), torch.sum(diff_vt, dim= [1, 2])
        scale_v = torch.nn.Softmax(1)(torch.cat((vv.unsqueeze(1), vt.unsqueeze(1), vt_neg.unsqueeze(1)), 1))
        ...
        return advise_t, advise_v     


    def similarity_matrix(self, emb_t, emb_v):
       ...
       # similarity_at = torch.matmul(emb_a_norm,emb_t_norm.permute(0, 2, 1))
        ...
        return  similarity_tv,  similarity_vt


    def forward(self, inputs):
        x_t, x_v = inputs[:, :self.length], inputs[:, self.length * 2:]
        self.batch = inputs.shape[0]

        x_t_, x_v_ = self.proj_t(x_t),  self.proj_v(x_v)
        advise_t, advise_a, advise_v = self.generate_advise(x_t_, x_v_)
        ...
        action = torch.cat((action_t, action_v), 1)
        return action


    def fused(self, state, action):    
        state_t, state_v = state[:, :self.length], state[:, self.length * 2:]     
        action_t, action_v = action[:, :self.length], action[:, self.length * 2:]
        ...
        state_new = torch.cat((x_t_o,x_v_o), 1)
        return state_new

    def temporal(self, inputs):
        x_t, x_v = inputs[:, :self.length], inputs[:, self.length * 2:]
        emb_t, emb_v = x_t.permute(1, 0, 2), x_v.permute(1, 0, 2)

        emb_t = self.temporal_t(emb_t)
        #emb_a = self.temporal_a(emb_a)
        emb_v = self.temporal_v(emb_v)

        emb_t, emb_v = emb_t.permute(1, 0, 2), emb_v.permute(1, 0, 2)

        ####classification
        emb_t_mean, emb_v_mean = torch.mean(emb_t, 1).squeeze(), torch.mean(emb_v, 1).squeeze()
        emo_predictions = self.classifier_e((emb_t_mean + emb_v_mean)/3)
        # emo_predictions = self.classifier_e(torch.cat((emb_t_mean, emb_a_mean, emb_v_mean), -1))
        return emo_predictions

class Critic(nn.Module): # estimate Q-value of the state evaluate how good an action from the Actor is given the current state
    def __init__(self, config):
    ...
    def forward(self, state, action):
        ...
        action_t, action_v = action_t.repeat(1, 1, self.n_head),action_v.repeat(1, 1, self.n_head)
        
        text, visual = torch.cat((state_t, action_t), -1), torch.cat((state_v, action_v), -1)

        text, visual,  = text.permute(1, 0, 2), visual.permute(1, 0, 2)
        ...
        text, visual = text.permute(1, 0, 2).mean(1),  visual.permute(1, 0, 2).mean(1)

        q = self.proj(torch.cat((text, visual), -1))
        return q.squeeze()
